...
import com.rxxb.imagepicker.R.dimen;
...
public class CropIwaOverlayConfig {
    ...
    public static CropIwaOverlayConfig createDefault(Context context) {
        ...
        CropIwaOverlayConfig config = (new CropIwaOverlayConfig()).setBorderColor(r.color(color.cropiwa_default_border_color)).setCornerColor(r.color(color.cropiwa_default_corner_color)).setGridColor(r.color(color.cropiwa_default_grid_color)).setOverlayColor(r.color(color.cropiwa_default_overlay_color)).setBorderStrokeWidth(r.dimen(dimen.cropiwa_default_border_stroke_width)).setCornerStrokeWidth(r.dimen(dimen.cropiwa_default_corner_stroke_width)).setCropScale(1.0F).setGridStrokeWidth(r.dimen(dimen.cropiwa_default_grid_stroke_width)).setMinWidth(r.dimen(dimen.cropiwa_default_min_width)).setMinHeight(r.dimen(dimen.cropiwa_default_min_height)).setAspectRatio(new AspectRatio(2, 1)).setShouldDrawGrid(true).setDynamicCrop(true);
    ...
    public static CropIwaOverlayConfig createFromAttributes(Context context, AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(attrs, styleable.CropIwaView);

            try {
                c.setMinWidth(ta.getDimensionPixelSize(styleable.CropIwaView_ci_min_crop_width, c.getMinWidth()));
                c.setMinHeight(ta.getDimensionPixelSize(styleable.CropIwaView_ci_min_crop_height, c.getMinHeight()));
                c.setAspectRatio(new AspectRatio(ta.getInteger(styleable.CropIwaView_ci_aspect_ratio_w, 1), ta.getInteger(styleable.CropIwaView_ci_aspect_ratio_h, 1)));
                c.setCropScale(ta.getFloat(styleable.CropIwaView_ci_crop_scale, c.getCropScale()));
                c.setBorderColor(ta.getColor(styleable.CropIwaView_ci_border_color, c.getBorderColor()));
                c.setBorderStrokeWidth(ta.getDimensionPixelSize(styleable.CropIwaView_ci_border_width, c.getBorderStrokeWidth()));
                c.setCornerColor(ta.getColor(styleable.CropIwaView_ci_corner_color, c.getCornerColor()));
                c.setCornerStrokeWidth(ta.getDimensionPixelSize(styleable.CropIwaView_ci_corner_width, c.getCornerStrokeWidth()));
                c.setGridColor(ta.getColor(styleable.CropIwaView_ci_grid_color, c.getGridColor()));
                c.setGridStrokeWidth(ta.getDimensionPixelSize(styleable.CropIwaView_ci_grid_width, c.getGridStrokeWidth()));
                c.setShouldDrawGrid(ta.getBoolean(styleable.CropIwaView_ci_draw_grid, c.shouldDrawGrid()));
                c.setOverlayColor(ta.getColor(styleable.CropIwaView_ci_overlay_color, c.getOverlayColor()));
                c.setCropShape((CropIwaShape)(ta.getInt(styleable.CropIwaView_ci_crop_shape, 0) == 0 ? new CropIwaRectShape(c) : new CropIwaOvalShape(c)));
                c.setDynamicCrop(ta.getBoolean(styleable.CropIwaView_ci_dynamic_aspect_ratio, c.isDynamicCrop()));
            } finally {
                ta.recycle();
            }
