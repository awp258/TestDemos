...
import com.jw.cameralibrary.CameraConfig.EXTRA_ITEMS
import com.jw.cameralibrary.CameraConfig.RESULT_CODE_ITEMS
...
class ShotRecordMainActivity : BaseBindingActivity<ActivityCameraBinding>() {
    private val CACHE_VIDEO_PATH = CameraConfig.CACHE_VIDEO_PATH //视频缓存路径
    private val CACHE_VIDEO_PATH_COVER = CameraConfig.CACHE_VIDEO_PATH_COVER //视频缓存路径
    private val CACHE_IMG_PATH = CameraConfig.CACHE_IMG_PATH //图片缓存路径
    ...
    override fun doConfig(arguments: Intent) {
        ...
        jCameraView!!.setFeatures(CameraConfig.SHOT_TYPE)
        if (CameraConfig.SHOT_MODEL == 2) {
            when (CameraConfig.SHOT_TYPE) {
        ...
        jCameraView!!.setJCameraLisenter(object :
            ...
            override fun captureSuccess(bitmap: Bitmap) {
                ...
                CropConfig.galleryAddPic(this@ShotRecordMainActivity, uri)
            }
        ...
        if (data?.extras != null) {
            when (requestCode) {
                ...
                CropConfig.REQUEST_CODE_ITEM_CROP -> {
                    val resultUri =
                        data.getParcelableExtra(CropConfig.EXTRA_CROP_ITEM_OUT_URI) as Uri
                ...
                //从视频编辑页面返回
                CropConfig.REQUEST_CODE_ITEM_CROP -> {
                    val path = data.getStringExtra(CropConfig.EXTRA_CROP_ITEM_OUT_URI)
